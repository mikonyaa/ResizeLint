...
public enum Fingerprinter {
    public static func fingerprint(
        ...
        let normalizedTokens = surroundingTokens
            .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
            .filter { !$0.isEmpty }
            .joined(separator: " ")
        let payload = [ruleID, normalize(path), syntaxKind, normalizedTokens].joined(separator: "\u{1F}")
        return "sha256:\(SHA256.hexDigest(Array(payload.utf8)))"
    }
